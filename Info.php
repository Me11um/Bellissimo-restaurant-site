<?php
    ...
    if ( isset($data['sendcomment']))
    {
        ...
        {
            $commenterrors[] = 'Введите отзыв!';
        }
        if ( isset($_SESSION['logged_user']))
        {
            
        } else {
            $commenterrors[] = 'Войдите, чтобы оставить отзыв!';
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Оставить отзыв о ресторане Bellissimo</title>
	<meta name="description" content="Здесь вы можете оставить свой отзыв о ресторане итальянской кухни Bellissimo">
	<meta name="keywords" content="Оставить отзыв, Итальянская кухня, Ресторан, Bellissimo">
...
<form action="/Info.php" id="signup-form" method="POST">
...
<div class="block">
<H1>Здесь вы можете оставить отзыв о нашем ресторане!</H1>
<form action="/Info.php" id="signup-form" method="POST">
<textarea class="commentarea" name="comment" placeholder="Оставьте отзыв..." rows=5 maxlength="500"></textarea>
<button class="commentbutton" name="sendcomment" type="submit">Отправить отзыв</button>
</form>
<?php
    if ( isset($data['sendcomment']))
    {
    if( empty($commenterrors) )
    {
        ...
        $user->name = $_SESSION['logged_user']->name;
        $user->text = $data['comment'];
        R::store($user);
        echo '<p style="color:#00FF00;">Отзыв был успешно отправлен!</p>';
    } else {
        echo '<p style="color:#FF0000;">'.array_shift($commenterrors).'</p>';
        }
    }
?>
<p align="left"></p>
...
<?php
    $user = R::find('comments', 'text != ""');
    foreach ($user as &$value) {
        echo '<div class="block">';
        echo '<img src="DefaultImage.png" style="position:relative; left:0%; width:10%; height:10%; padding:0; padding-bottom: 8%; margin:0;">';
        echo '<div align="left" style="position:relative; float:right; width:85%; padding:0; padding-bottom: 2%; margin:0; top:1px; font-size:18px;">'.$value->name.':<br><br>'.$value->text.'</div>';
        echo '<HR align="center" color=#E8E8E8></div>';
    }
?>
</div>
</center>
